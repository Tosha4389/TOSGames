...
{    
    ...
    {        
        if(gameManager.playerHP < 100 && gameManager.playerHP >=0) {
            greenBar.SetActive(true);
            redBar.SetActive(true);           
        }
        scaleBar = greenBar.transform.localScale;        
        greenBar.transform.localScale = new Vector3(gameManager.playerHP / 100f, scaleBar.y, scaleBar.z);
        greenBar.transform.localPosition = new Vector3((1f - gameManager.playerHP / 100f) / -2f, 0f, 0f);
    }
