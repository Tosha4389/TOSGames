...
public class GarageManager : MonoBehaviour
{
    ...
    {
        loadParam.speedRotateTurretLoad = turretUpd[0].speedRot;
        turretRend.sprite = turretSprites[0];

        loadParam.playerDamageLoad = gunUpd[0].damage;
        loadParam.fireRateLoad = gunUpd[0].fireRate;
        loadParam.scatterLoad = gunUpd[0].scatter;
        loadParam.accuracyLoad = gunUpd[0].accuracy;
        gunRend.sprite = gunSprites[0];
        gunPos.y = 0.62f;
        tankGun.transform.localPosition = gunPos;
        loadParam.gunPos = gunPosStart;

        loadParam.speedMoveLoad = chassisUpd[0].speedMove;
        loadParam.speedRotateLoad = chassisUpd[0].speedRot;
        chassisRend.sprite = chassisSprites[0];
    }
